...
class ConnectorSnippet(models.TransientModel):
    ...
    def sync_ecomm_category(self, exp_category_objs, channel, instance_id, sync_opr, connection):
        ...
        if sync_opr == 'export':
            for exp_category_obj in exp_category_objs:
                categ_id = self.sync_categories(
                    exp_category_obj, instance_id, channel, connection)
                success_ids.append(exp_category_obj.id) if categ_id else failed_ids.append(
                    exp_category_obj.id)
        else:
            action = 'c'
            success_ids, failed_ids, updted_category_mapping_ids = self._update_specific_category(
                exp_category_objs, channel, instance_id, connection)
            if success_ids:
                self.update_odoo_mapping(
                    'connector.category.mapping', updted_category_mapping_ids, {'need_sync': False})
        if success_ids:
            text = "\nThe Listed category ids {} has been {} on {}.".format(
        ...
        if failed_ids:
            text = "\nThe Listed category ids {} have not been {} on {}.".format(
    ...
    def sync_categories(self, category_obj, instance_id, channel, connection):
        if category_obj:
            mapped_category_objs = category_obj.connector_mapping_ids.filtered(
                lambda obj: obj.instance_id.id == instance_id)
            if not mapped_category_objs:
                parent_categ_id = self.sync_categories(
                    category_obj.parent_id, instance_id, channel, connection) if category_obj.parent_id else 1
                if hasattr(self, 'create_%s_category' % channel):
                    name = category_obj.name
                    odoo_id = category_obj.id
                    response = getattr(self, 'create_%s_category' % channel)(
                        odoo_id, parent_categ_id, name, connection)
    ...
    def _update_specific_category(self, updt_map_objs, channel, instance_id, connection):
        ...
        for categ_map_obj in updt_map_objs:
            ...
            if categ_obj and ecomm_id:
                ecomm_cat_parent_id = self.get_ecomm_parent_id(
                    categ_obj, instance_id, channel, connection)
    ...
    def get_ecomm_parent_id(self, categ_obj, instance_id, channel, connection):
        ...
        if categ_obj.parent_id:
            parent_category_obj = categ_obj.parent_id.connector_mapping_ids.filtered(
                lambda obj: obj.instance_id.id == instance_id)
