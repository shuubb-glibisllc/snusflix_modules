...
class ConnectorSnippet(models.TransientModel):
    ...
    def get_map_updt_attribute_objs(self, instance_id):
        attribute_map_objs = self.env['connector.attribute.mapping'].search(
    ...
    def sync_ecomm_attribute(self, connection):
        ...
        if attribute_objs:
            ...
            for attribute_obj in attribute_objs:
                response_text, attribute_count = self.start_sync_oprations(
                    attribute_obj, map_dict, channel, instance_id, connection, response_text=response_text, attribute_count=attribute_count)
        else:
            response_text = "No Attribute(s) Found To Be Export!!!"
        if attribute_count:
            response_text = "{}\n {} Attribute(s) and their value(s) successfully Synchronized".format(
                response_text, attribute_count)
        return response_text

    def start_sync_oprations(self, attribute_obj, map_dict, channel, instance_id, connection, **kwrgs):
        response_text, attribute_count = kwrgs.get(
            'response_text', ''), int(kwrgs.get('attribute_count', 0))
        ...
        if odoo_id not in map_dict.keys():
            if hasattr(self, 'create_%s_product_attribute' % channel):
                ecomm_attribute_code = name.lower().replace(
                    " ", "_").replace("-", "_")[:29]
                ecomm_attribute_code = ecomm_attribute_code.strip()
                response = getattr(self, 'create_%s_product_attribute' % channel)(
                    name, odoo_id, connection, ecomm_attribute_code)
                if response.get('status', False):
                    ecomm_id = response.get('ecomm_id', 0)
                    if response.get('ecomm_attribute_code'):
                        ecomm_attribute_code = response.get(
                            'ecomm_attribute_code')
                    if ecomm_id:
                        self.create_odoo_connector_mapping(
                            'connector.attribute.mapping', ecomm_id, odoo_id, instance_id, ecomm_attribute_code=ecomm_attribute_code)
                        self.create_ecomm_connector_mapping('connector.attribute.mapping', channel, {
                                                            'ecomm_id': ecomm_id, 'odoo_id': odoo_id, 'name': ecomm_attribute_code, 'created_by': 'odoo'}, connection)
                else:
        ...
        if ecomm_id:
            if hasattr(self, 'create_%s_product_attribute_value' % channel):
                getattr(self, 'create_%s_product_attribute_value' % channel)(
                    ecomm_id, attribute_obj, ecomm_attribute_code, instance_id, connection)
            attribute_count += 1
