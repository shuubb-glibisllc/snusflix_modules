...
class SaleOrder(models.Model):
    ...
    def _invoiced_status_compute(self):
        for sale_obj in self:
            sale_obj.is_invoiced = True if sale_obj.invoice_ids and all(invoice_id.state == 'posted' for invoice_id in sale_obj.invoice_ids) and all(
    ...
    def unlink(self):
        mappings = self.env['connector.order.mapping'].search(
            [('odoo_order_id', 'in', self.ids)])
        self.env['connector.snippet'].delete_connector_mapping(
            'connector.order.mapping', self, 'Order', mappings)
        return super().unlink()
