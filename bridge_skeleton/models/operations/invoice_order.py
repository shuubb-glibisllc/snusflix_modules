    ...
    def set_order_paid(self, paymentData):
        ...
        try:
            ...
            statusMessage = "Error in creating Payments for Invoice: %s" % str(
                e)
            _logger.debug('## Exception set_order_paid for sale.order(%s) : %s' % (
                paymentData.get('order_id'), statusMessage))
            status = False
        finally:
            return {
        ...
        try:
            ...
            if not invoiceId:
                ...
                if ecommerceInvoiceId and invoiceId:
                    invoiceId.write(
                        {'name': ecommerceInvoiceId, 'ecomm_invoice': ecommerceInvoiceId})
            else:
                statusMessage = "Invoice Already Created"
        except Exception as e:
            status = False
            statusMessage = "Error in creating Invoice: %s" % str(e)
            _logger.debug('## Exception create_order_invoice for sale.order(%s) : %s' % (
