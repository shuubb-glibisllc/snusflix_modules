...
class WkSkeleton(models.TransientModel):
    ...
    def set_order_shipped(self, orderId):
        ...
        try:
            ...
            if saleObj.picking_ids:
                for pickingObj in saleObj.picking_ids.filtered(
                    ...
                    for packObj in pickingObj.move_ids_without_package:
                        if packObj.quantity and packObj.quantity < packObj.product_uom_qty:
                            backorder = True
                            continue
                        elif packObj.product_uom_qty > 0:
                            packObj.write(
                                {'quantity': packObj.product_uom_qty})
                        else:
                            packObj.unlink()
                    if backorder:
                        ...
                        if instance_id:
                            instance_obj = self.env['connector.instance'].browse(
                                instance_id)
                            sms_obj = self.env['confirm.stock.sms'].create(
                                {'pick_ids': [(6, 0, [pickingObj.id])]})
            ...
            _logger.debug('## Exception set_order_shipped(%s) : %s' %
                          (orderId, status_message))
        finally:
            return {
