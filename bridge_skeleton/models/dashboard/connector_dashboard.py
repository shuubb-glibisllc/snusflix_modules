...
class ConnectorDashboard(models.Model):
    ...
    def get_instance(self):
        ...
        check_dashboard = connector_instance.sudo().search(
            [('active', '=', True), ('current_instance', '=', True)])
    ...
    def _compute_record_count(self):
        for single_record in self:
            ...
            if action == 1:
                ...
                single_record[needTwo] = len(totlaNeedDlvr)
            ...
            single_record.count_mapped_records = self._get_mapped_records(
    ...
    def get_bar_graph_datas(self, itemName):
        ...
        color = '#7c7bad'
        graphData = [{'values': data, 'area': True, 'title': '',
                      'key': itemName.title(), 'color': color}]
        return graphData
