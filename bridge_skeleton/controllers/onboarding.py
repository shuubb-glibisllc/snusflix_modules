...
class OnboardingController(http.Controller):
    ...
    def infos(self):
        ...
        if instance_id:
            instance_id = request.env['connector.instance'].sudo().browse(
                instance_id)
            extensions = request.env['connector.snippet'].sudo(
            )._get_ecomm_extensions()
            current_ecomm = [i for i in extensions if i[0]
            ...
            href_data = request.env['wk.skeleton'].get_ecomm_href(
                current_ecomm[0][0])
            if href_data:
            ...
            infos['status'] = instance_id.connection_status
        else:
            infos.update({
                'instance_detail': ('', ''),
                'current_ecomm': [('', '')],
            })
        infos.update(self.get_all_instances())
        return infos

    def get_all_instances(self):
        allInstances = request.env['connector.instance'].sudo().search([])
        instance_list = []
        for instance in allInstances:
            instance_list.append((instance.id, instance.instance_name))
        return {'all_instances': instance_list}

    @http.route('/bridge_skeleton/skeleton_instance_set', type='json', auth='user')
    def skeleton_instance_set(self, instance_id=None):
        check_dashboard = request.env['connector.instance'].sudo().search(
            [('current_instance', '=', True)])
        check_dashboard.sudo().write({'current_instance': False})
        if instance_id:
            instance_search = request.env['connector.instance'].sudo().browse(
                int(instance_id))
    ...
    def skeleton_channel_set(self, channel_id=False):
        ...
        if instance_ids:
            ...
            instance_id = request.env['connector.dashboard'].sudo(
            ).get_instance()
            if instance_id:
                return instance_id
        return False
    ...
    def get_instances(self, channel_id):
        if channel_id:
            instance_ids = request.env['connector.instance'].sudo().search(
                [('ecomm_type', '=', channel_id)])
            return instance_ids
        return False
